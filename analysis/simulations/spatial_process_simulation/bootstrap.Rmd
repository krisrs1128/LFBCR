---
title: "Compare Bootstraps on the Spatial Process Simulation"
...
features were previously learned and extracted from the spatial point process
simulation.
...
this analysis script and the one in `analysis/data_analysis/`. This is because
they both apply each type of bootstrap to output a `csv` file of aligned
projections that will appear in the `params$derived_data` folder.
...
```{r}
prefix <- ifelse(str_detect(params$subset, "rcf"), "linear_best.npy", "*best.npy")
...
To run this notebook across all spatial process relevant settings, you can use
the following in the shell in the folder containing this `.Rmd` file. The first
two `Rscript`'s run across the CNN and VAE features, and the last is for the
RCF.

```{sh, eval = FALSE}
for s in 15 5 95; do
  for k in 32 64 128; do
    Rscript -e "rmarkdown::render('bootstrap.Rmd', params = list(subset='cnn-k${k}-${s}', transform='log'))"
    Rscript -e "rmarkdown::render('bootstrap.Rmd', params = list(subset='vae-k${k}-${s}')"
  done;

  for k in 256 512 1024; do
    Rscript -e "rmarkdown::render('bootstrap.Rmd', params = list(subset='rcf-k${k}-${s}')"
  done;
done;
```
