...
params:
  run: "../../data/raw_data/spatial_process_models/vae-k128-50.yaml_0.tar.gz"
...
Xy <- read_csv(list.files(params$out_dir, "Xy*", full = TRUE)) %>%
  left_join(subset)
H_ <- np$load(list.files(fdir, "*best.npy", full = TRUE)) %>%
  as.data.frame()
```

```{r, out.width = 900, out.height = 400}
cur_paths <- subset %>%
...
png(file.path(out_dir, str_remove(basename(params$run))))
superheat(
  t(cor(H_, H[, -1])),
